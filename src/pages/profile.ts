...
function renderCopyPanel(data){
  ...
  panel.innerHTML = items.map(([k, label]) => \`
    <div class="flex items-center gap-2 bg-white/5 rounded-xl px-3 py-2">
      ...
      <button type="button" onclick="copyVal(this, '\${k}')" data-val="\${String(data[k]).replace(/"/g,'&quot;')}" aria-label="\${label} কপি করুন" title="\${label} কপি করুন" class="text-xs bg-orange-500/20 hover:bg-orange-500/50 text-orange-300 px-2.5 py-1.5 rounded-lg transition"><i class="fas fa-copy"></i></button>
    </div>\`).join('');
}
